...
function AddProduct() {
  ...
  const getCategory = async () => {
    try {
      const response = await axios.get("http://42store.duynguyen23.io.vn:5000/api/category");
  ...
  const handleImageChange = (e, variantIndex) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        axios
          .post("http://42store.duynguyen23.io.vn:5000/upload", formData, {
            headers: {
  ...
  const handleSubmit = async() => {
    ...
    try {
      ...
      const response = await axios.post("http://42store.duynguyen23.io.vn:5000/api/product", payload);
      if(response){
