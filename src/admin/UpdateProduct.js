...
function UpdateProduct() {
  ...
  const getCategory = async () => {
    try {
      const response = await axios.get("hhttp://42store.duynguyen23.io.vn:5000/api/category");
  ...
  const fetchProductDetails = async () => {
    try {
      const response = await axios.get(
        `hhttp://42store.duynguyen23.io.vn:5000/api/product/${productId}`
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `hhttp://42store.duynguyen23.io.vn:5000/api/product`,
        payload
      );
